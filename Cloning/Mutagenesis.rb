...
class Protocol
	...
	def main
        ...
        reactions_to_use = [operations.running.count, 3].max
        
        show do
            title "Make master mix"
            check "Get out GeneArt® Site-Directed Mutagenesis PLUS Kit from the -20C freezer."
            ...
            table_matrix[0] = ["Water to add", "10X Accuprime Pfx Reaction Buffer to add", "10X Enhancer to add", "DNA Methylase to add", "25X SAM to add", "Accuprime Pfx to add"]
            table_matrix[1] << {content: "#{(reactions_to_use*14.447).round(2)} µl", check: true}
            table_matrix[1] << {content: "#{(reactions_to_use*2.2).round(2)} µl", check: true}
            table_matrix[1] << {content: "#{(reactions_to_use*2.2).round(2)} µl", check: true}
            table_matrix[1] << {content: "#{(reactions_to_use*0.44).round(2)} µl", check: true}
            table_matrix[1] << {content: "#{(reactions_to_use*0.88).round(2)} µl", check: true}
            table_matrix[1] << {content: "#{(reactions_to_use*0.183).round(2)} µl", check: true}
            table table_matrix
            check "Return GeneArt® Site-Directed Mutagenesis PLUS Kit to the freezer."
        end
		...
		op_array.each do |zones_in_block|
		    ...
		    show do
                ...
                bullet "Select \"Open Method\", then \"Mutagenesis GeneArt PLUS Kit\" (located in public folder)"
		...
		show do
		    title "Transfer 2 µl to 1.5 ml tubes"
		    check "Take reactions out of the thermocycler and place on ice."
		    check "Transfer 2 µl of liquid from the small PCR tubes to new 1.5 ml tubes and label with the same IDs:"
		    table operations.start_table
		        .custom_column(heading: "New 1.5 ml tube ID") { |op| op.output(PRODUCT).item.id}
		        .custom_column(heading: "Mutagenesis reaction (small PCR tube) to add") {|op| {content: "2 µl of #{op.output(PRODUCT).item.id}", check: true}}
		        .end_table
            check "Discard small PCR tubes."
		end
		
		show do
		    title "Assemble reactions"
		    check "Get out GeneArt® 2X Enzyme Mix in the -80C freezer."
		    check "Add water and master mix to tubes according to the following table:"
		    table operations.start_table
		        .output_item(PRODUCT)
		        .custom_column(heading: "Water to add") {{content: "8 µl of water", check: true}}
		        .custom_column(heading: "GeneArt® 2X Enzyme Mix to add") {{content: "10 µl of 2X Enzyme Mix", check: true}}
		        .end_table
            check "Return GeneArt® 2X Enzyme Mix to the freezer."
		end
