...
class Protocol
  ...
  def main
    ...
    show do
        title "Place items in sample pick up rack in -20C."
        check "Place items in sample pick up rack in -20C."
    ...
    items_by_user.each do |user, items|
        ...
        The following items are ready for pickup from the DAMP Lab North (Room 423, 610 Commonwealth Ave. Boston, MA 02215). Please email #{Parameter.get_string('smtp_email_address')} for questions or more information.<br/><br/>"
        file_paths = {}
        items.each do |item|
        ...
        send_email user.name, (user.parameters.find { |p| p.key == 'email'}.value), "DAMP Lab items ready for pickup", body, file_paths if user.parameters.find { |p| p.key == 'email'}
    end
